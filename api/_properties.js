...
async function available() {
  ...
  try {
    ...
    if (!r.ok) {
      ...
      console.warn(`[panden] tabel "${TABEL}" niet leesbaar (HTTP ${r.status}) -- over 30 s opnieuw.`);
    } else {
      _beschikbaarReden = '';
    }
  } catch (e) {
    console.warn('[panden] Airtable onbereikbaar (over 30 s opnieuw):', e && e.message);
    _beschikbaar = false;
    _beschikbaarTot = Date.now() + HERPROBEER_MS;
...
module.exports = {
  ...
  PropertyError,
  available, configured, _resetAvailability, onbeschikbaarReden,
